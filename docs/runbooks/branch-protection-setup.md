...
| `lint-orphan-bem`                                                       | [`quality.yml`](../../.github/workflows/quality.yml)                                  | ADR-0007 orphan-BEM gate (`node scripts/lint-orphan-bem.mjs`). Fails when an `apps/web/src/**` `.astro`/`.tsx` file carries a BEM-shaped class name (`block__element` / `block--modifier`) that does not resolve via a colocated `<style>` / `global.css` / `cva` variant / colocated `<script>` JS hook. **Escape hatch**: refactor the class into Tailwind utilities, or — for a pre-existing orphan — add a justified entry to [`.lint-orphan-bem-allowlist.json`](../../.lint-orphan-bem-allowlist.json) ([ADR-0007](../decisions/0007-ui-styling-convention.md)). |
| `lint-astro-frontmatter`                                                | [`quality.yml`](../../.github/workflows/quality.yml)                                  | Story #865 — fails when an `apps/web/src/**` `.astro` file's frontmatter (`---` ... `---` block) contains the literal `<script` token in a comment. Astro's compiler treats the token as a real script-block opener, producing `pnpm dev` parser errors on downstream consumers (PRs #862 and #864). **Escape hatch**: reword the comment to use the word `script` without angle brackets. |
...
| `knip strict (no baseline, ADR-0006)`                                   | [`quality.yml`](../../.github/workflows/quality.yml)                                  | Full `knip` pass — unused files, exports, dependencies. **Escape hatch**: fix the finding (no baseline) or list the dep in `knip.config.ts` `ignoreDependencies` with a reason ([ADR-0006](../decisions/0006-local-hook-stack.md)). |
| `Markdown lint (ADR-0006)`                                              | [`quality.yml`](../../.github/workflows/quality.yml)                                  | `markdownlint-cli2` against every `.md` outside the standard ignore set. **Escape hatch**: fix the rule violation or relax the rule in `.markdownlint.jsonc` with a comment naming the reason ([ADR-0006](../decisions/0006-local-hook-stack.md)). |
| `secretlint (ADR-0006)`                                                 | [`quality.yml`](../../.github/workflows/quality.yml)                                  | `@secretlint/secretlint-rule-preset-recommend` against the full tree; CI mirror of the local pre-commit gate. **Escape hatch**: rotate the leaked credential and add a justified entry to `.secretlintignore` ([ADR-0006](../decisions/0006-local-hook-stack.md)). |
