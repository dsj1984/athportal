...
## ADR-020 — Renovate is the maintenance channel for `github-actions` SHA pins

**Status**: Accepted (2026-05-19, Epic #386)

**Context**: Story #408 introduced SHA-pinned `uses:` references for every third-party GitHub Action invoked from `.github/workflows/**` (supply-chain hardening: pinning to a commit SHA prevents a compromised tag from re-pointing at malicious workflow code). SHA pins are a one-shot fix unless an automated channel keeps them current — without scheduled upkeep, the pins decay silently as upstream Actions ship security patches the workflows never adopt. The repo already uses Renovate as its scheduled dependency-update bot per [`docs/decisions/0005-dependency-update-posture.md`](./decisions/0005-dependency-update-posture.md), and Renovate's `config:recommended` preset enables the canonical default managers — which include the `github-actions` manager — so the upkeep machinery is already wired; this ADR pins the policy choice in writing so a future config refactor can't drop the manager without superseding it.

**Decision**:

- **Renovate is the maintenance channel for `github-actions` SHA pins.** Renovate's `github-actions` manager reads `uses:` lines under `.github/workflows/**` and `.github/actions/**`, recognises the SHA-pinned shape (`owner/repo@<40-char-sha> # vX.Y.Z`), and proposes update PRs that bump both the SHA and the trailing version comment in lockstep. Existing `packageRules` from `renovate.json` apply: patch and minor updates auto-merge per the `matchUpdateTypes: ["patch", "minor"]` rule; major updates require dashboard approval per the `matchUpdateTypes: ["major"]` rule. The weekly Monday window from ADR-0005 governs scheduling.
- **The `github-actions` manager is enabled via `config:recommended`, not via `enabledManagers`.** Renovate's `config:recommended` preset enables the default manager set (npm, github-actions, nvm, dockerfile, …); we do not enumerate `enabledManagers` because that would convert an additive preset into a manual allowlist that the next contributor adding a manager would have to discover and extend. `pnpm dlx --package=renovate -- renovate-config-validator renovate.json` exits 0 against the current config — the validator is the binding check.
- **`renovate.json` is the single source of truth.** All Renovate behaviour (schedule, automerge tiers, vendor-family groups, lockfile maintenance, vulnerability-alert posture) lives in the committed config. Hosted Renovate consumes the file as-is; there is no out-of-band UI configuration that overrides it.

**Rejected — explicit `"enabledManagers": ["npm", "github-actions"]`**: Converts the additive `config:recommended` preset into a closed allowlist. Future managers added by Renovate to the recommended set (or managers the repo gains naturally, e.g. `dockerfile` once container images land) would need a paired `enabledManagers` edit or they would silently no-op. The current shape inherits the recommended default and is the lower-maintenance posture.

**Rejected — separate Dependabot config for `github-actions`**: Splitting the update channel across two bots doubles the PR-review surface and breaks the vendor-family grouping (`matchPackageNames` rules and the weekly window cannot apply across two bots). ADR-0005 already chose Renovate as the scheduled bot; this ADR is the consistent application of that choice to a new manager.

**Consequences**:

- SHA pins introduced by Story #408 stay current automatically. A compromised upstream tag is bounded by the time between the malicious push and Renovate's next scheduled run, modulated by the auto-merge tier.
- The validator (`pnpm dlx --package=renovate -- renovate-config-validator renovate.json`) is the canonical local check that the config still parses. CI does not run it today — that surface lands with a future Epic if it becomes load-bearing.
- Cross-references: [`docs/decisions/0005-dependency-update-posture.md`](./decisions/0005-dependency-update-posture.md) is the parent ADR that named Renovate as the scheduled bot; this ADR extends that decision to the `github-actions` manager surface introduced by Story #408 (SHA-pinning the action set).
