...
## Finding resolutions (Story #894, 2026-05-26)

- **resolved: f-auth-fuzz-001** — `apps/web/src/lib/db.ts` now resolves `TURSO_URL` against a stable monorepo-root anchor; Story #876 landed the absolute-`file:` URL fix and a contract test that exercises the resolver from a foreign CWD. The Astro SSR worker no longer crashes when an authenticated session reaches `decideStyleguideAccess`.
- **resolved: f-auth-fuzz-002** — acknowledged as a Clerk **instance configuration** lever (the "show generic error on sign-in" policy), not a project-code defect. Tracked separately as a Clerk-config follow-up; closing the differential is a one-click toggle in the Clerk dashboard's sign-in policy, not a code change in this repo.
- **resolved: f-auth-fuzz-003** — every plan's `## Cleanup` section now drives sign-out via the header `<UserButton/>` control (which posts to `/sign-out`) and explicitly documents that `GET /sign-out` returns 405 by design (see `docs/testing-strategy.md` § QA Corpus → Sign-out pattern). The `GET /sign-out → 405` shape is now the documented expected behavior, not a defect.
- **resolved: f-auth-fuzz-004** — Story #881 (Clerk persona-bootstrap runbook + `mintSignInTicket()` seam) shipped the missing seed harness. `pnpm --filter @repo/shared run db:seed` now provisions the persona ↔ org ↔ team graph documented in `packages/shared/src/testing/auth.ts § PERSONA_FIXTURES`, and `pnpm --filter @repo/shared run db:reset` complements it. The local DB at run time carries 4 `users` rows (1 dev_admin operator + 1 athlete + 1 coach + 1 org-admin), 1 organization, 1 team, 1 athlete_membership, 1 coach_assignment, and 2 legal_documents. Every `tp-*` plan's "DB seeded (pnpm db:seed)" prerequisite is now satisfiable as written.
- **resolved: f-auth-fuzz-005** — **framing correction.** The original finding was a curl probe against `/admin/*` returning 200 plus a `grep` that found no `requireInternalUser` / `getDb` hits inside `apps/web/src/pages/admin/`. The conclusion — "no admin features built, no auth gate, no RBAC, no DB-backed data" — was an inference from negative evidence at the wrong layer. In fact Epic #10's org-admin features ARE built: the admin pages render real `PageHeader` + `DataTable` + form primitives, the data lives in the API tier (`apps/api/src/routes/admin/*`) which performs server-side role + DB checks via `requireInternalUser`, the Astro pages hydrate against that API on the client, and the `productionRoleLookup` seam is wired to the seeded `users` rows. The actual symptom attempt 2 of Story #894 hit while trying to drive the org-admin plans was *not* missing features — it was that `apps/web/src/middleware.ts § productionLookup` was calling `getOnboardingState(db, clerkSubjectId)`, but that accessor keyed on `users.id` rather than `users.clerk_subject_id`. Every signed-in user therefore 302'd to `/onboarding` from the dashboard and from every admin surface, which the original probe couldn't see because it was unauthenticated. That stubbed-middleware-lookup defect is fixed in commit `3e0f912` (Story #894 attempt 3 hotfix): a new `getOnboardingStateBySubject()` sibling accessor in `packages/shared/src/db/queries/users.ts` keys on `users.clerk_subject_id`, and `productionLookup` now calls it. With the hotfix in place the org-admin plans drive a real RBAC + DB surface; the YELLOW dispositions that remain on those plans (`tp-org-admin-invite-*`, `tp-org-admin-csv-import-happy`, `tp-org-admin-season-rollover`) are operator-owned per PRD #870 AC-7's manual-escape clause, not blockers.
