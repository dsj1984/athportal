...
//
// Contract test for the direct-athlete-invitation endpoint
// (Epic #10 / Story #662 / Task #680).
//
// Pins the wire-shape invariants for
// `POST /api/v1/admin/invitations/athlete`:
//
//   1. Happy path — actor's org owns the named team. Returns 201
//      with `{ success: true, data: { id, email, role: 'athlete',
//      teamIds: [teamId], status: 'pending', createdAt } }`. The
//      Clerk wrapper is called with the actor's orgId,
//      `role: 'athlete'`, and `teamIds: [teamId]`. The local
//      `invitations` row carries the same triple plus the new Clerk
//      id and is scoped to the actor's org.
//
//   2. Cross-org isolation — actor from org A naming a team in org
//      B is refused with `404 NOT_FOUND` (no cross-tenant existence
//      oracle). No Clerk call fires and no local row is inserted.
//
//   3. Missing team — actor's org but a teamId that does not exist
//      is also `404 NOT_FOUND`. Same wire shape as cross-org.
//
//   4. Invalid body — empty body, missing fields, or a forged
//      `role` field are rejected with `400 INVALID_BODY`.
//
// The migration list mirrors `management.contract.test.ts` so the
// schema rev stays in sync across the invitations contract surface.
...
describe('POST /api/v1/admin/invitations/athlete — contract', () => {
  let clerk: ClerkStub;
  beforeEach(() => {
    clerk = makeClerkStub();
  });

  it('creates an athlete invitation pinned to a team the actor owns', async () => {
    const db = freshProductionDb();
    const seed = seedGraph(db);
    const app = buildApp(db, actorFor(seed, 'A'), clerk);

    const res = await app.request('/api/v1/admin/invitations/athlete', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ email: 'invitee@test.invalid', teamId: seed.teamA1 }),
    });
    ...
    expect(body.data.email).toBe('invitee@test.invalid');
    expect(body.data.role).toBe('athlete');
    expect(body.data.teamIds).toEqual([seed.teamA1]);
    ...
    expect(call?.emailAddress).toBe('invitee@test.invalid');
    ...
    expect(row?.teamIds).toEqual([seed.teamA1]);
    expect(row?.clerkInvitationId).toBe('inv_clerk_new_1');
    expect(row?.status).toBe('pending');
    expect(row?.invitedByUserId).toBe(seed.adminA);
  });

  it('refuses cross-org teamId with 404 NOT_FOUND and does not call Clerk or insert', async () => {
    const db = freshProductionDb();
    const seed = seedGraph(db);
    // Actor is org A; teamB1 belongs to org B.
    const app = buildApp(db, actorFor(seed, 'A'), clerk);

    const res = await app.request('/api/v1/admin/invitations/athlete', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ email: 'invitee@test.invalid', teamId: seed.teamB1 }),
    });
  ...
  it('returns 404 NOT_FOUND for a teamId that does not exist anywhere', async () => {
    ...
    const res = await app.request('/api/v1/admin/invitations/athlete', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ email: 'invitee@test.invalid', teamId: 'team_does_not_exist' }),
    });
